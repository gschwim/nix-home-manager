# devshells/python313-poetry.nix
#
# Thin Poetry-first devShell for Python 3.13 (recommended default for new projects).
# One file per devshell (user preference) for easy review and maintenance.
#
# Usage:
#   nix develop .#python313-poetry
#   # or via direnv:  use flake .#python313-poetry
#
# Design:
...
pkgs.mkShell {
  name = "python313-poetry";

  packages = with pkgs; [
    python313
    poetry
    python313.pkgs.pip   # explicit pip for ad-hoc needs inside the shell
  ...
  shellHook = ''
    echo "🐍 Python 3.13 + Poetry devShell (nixpkgs-25.11 / 26.05-darwin pin)"
    echo ""
