...
Prefers exact hostname match in homeConfigurations.
Falls back to desktop/role logic (linux-x86-desktop / linux-x86 or darwin equivalents).
EOF
        }
        ...
        if [[ -f "$HOST_INFO_FILE" ]]; then
          while IFS='=' read -r key value || [[ -n "$key" ]]; do
            ...
            case "$key" in
              ...
              desktop_environment) desktop_environment="$value" ;;
              role) role="$value" ;;
            esac
          done < "$HOST_INFO_FILE"
        fi
        ...
        case "$cmd" in
          switch)
            ...
            echo "homectl: using target '$target' (from hostname match or role/desktop + os logic)"
            mkdir -p "$STATE_DIR"
